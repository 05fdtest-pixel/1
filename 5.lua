...
-- Создаем стены и пол
local wallColor = colors.gray
local floorColor = colors.green
local pillarColor = colors.lightGray

-- Размеры 3D комнаты
local roomSize = 10
local halfSize = roomSize / 2

-- Пол и потолок (3D плоскости)
local floor = c3d.plane(roomSize, roomSize)
floor.pos = {0, 0, 0}
floor.color = floorColor
scene:add(floor)

-- 4 Столба (3D Кубы) на равном расстоянии друг от друга и от стен
...
for _, pos in ipairs(positions) do
    local pillar = c3d.cube(pillarSize, 2, pillarSize)
    pillar.pos = pos
...
while true do
    ...
    -- Рендер 3D кадра
    display:clear(colors.cyan) -- Цвет неба
    display:render(scene)
    display:draw()

    -- Получение команд с беспроводного мода
