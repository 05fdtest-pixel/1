...
-- Инициализация дисплея C3D
local display = c3d.new_display(monitor)
local scene = c3d.new_scene()

-- Камера
local camera = c3d.new_camera()
camera.fov = 70
camera.pos = {0, 1, 0}
camera.rot = {0, 0, 0}
...
-- Пол (3D plane)
local floor = c3d.new_plane(roomSize, roomSize)
floor.pos = {0, 0, 0}
...
for _, pos in ipairs(positions) do
    local pillar = c3d.new_cube(pillarSize, 2, pillarSize)
    pillar.pos = pos
